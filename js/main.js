...
const translations = {
    ...
    ar: {
        ...
        buy_now: "اشتري الآن",
        ai_btn: "✨ جرب المنتج (بالذكاء الاصطناعي)",
                    ...
                    ${subCats.map(sub => `
                        <div class="home-cat-card" onclick="window.openCategoryView('${sub.id}')">
                            ...
                            <div style="position: absolute; bottom: 18px; left: 18px; right: 18px; display: flex; flex-direction: column; gap: 8px;">
                                ...
                                <div style="display: flex; align-items: center; gap: 6px; font-size: 0.6rem; font-weight: 800; color: var(--primary); background: rgba(255,255,255,0.08); width: fit-content; padding: 4px 12px; border-radius: 50px; border: 1px solid rgba(212,175,55,0.3); white-space: nowrap;">
                                    <span data-i18n="shop_now">${currentLang === 'ar' ? 'اكتشف الآن' : 'EXPLORE NOW'}</span>
                                    <i class="fas fa-arrow-right" style="font-size: 0.5rem; transition: transform 0.3s;"></i>
                                </div>
                            </div>
                        </div>
                    `).join('')}
